...
NIGHTS.n3={
  ...
  obj:[
    ...
    {id:'crack',t:'فك تشفير <code>session_cookies.enc</code> واستخرج بيانات الدخول'},
    {id:'login',t:'سجّل الدخول لحسابه واعثر على الرسالة'},
    {id:'sent',t:'أرسل التقرير — القبض'}
  ],
  hints:{
    lynx1:'افتح تطبيق LYNX من شريط المهام وابحث: مهند الحسني — ثم ابحث: ليث',
    social:'في نتيجة lynx: عمّق بطاقة مهند — ستجد حسابه المجمد وشبكته المنزلية MOHANNAD-HOME.',
    router:'افتح المتصفح ← mohannad-home.net ← ثغرة RT-88 الموثقة في 0day-souq. hydra على راوتره: hydra -l admin -P rockyou.txt ssh://192.168.88.1 — ثم hijack من صفحة الراوتر، ثم cookiesteal.',
    crack:'افتح DECRYPT على الملف session_cookies.enc — نفس حلقة الإزاحة القديمة.',
    login:'المتصفح ← social.mohannad/login ← أدخل ما استخرجت من الكوكيز.',
    sent:'زر «إرفاق الأدلة وإرسالها» في اللوحة، أو send في الطرفية.'
  },
...
const ST_BEATS={
  ...
  n3:{
    async start(){
      ...
      await say('راصد. الرنة التي وصلتك ليست مني… ولا من ليث. تنصت لنا شخص ثالث، ولا وقت للأسئلة.');
      await say('اعتراضك لليلة الماضية أعطانا تردد مهند — لكن الإشارة لا تدين أحداً أمام المحكمة. نحتاج حياته الرقمية: علاقاته، أماكنه، خططه.');
      await say('وصلت محطتك أداة جديدة: LYNX — تجمع كل ما يُعرف عن أي اسم من الشبكة المفتوحة. افتحها من شريط المهام وابحث: مهند الحسني. ثم ابحث: ليث.');
      setPhoneView('chat');
    },
    async dossiers(){
      if(!S.flags.live)return;
      await sleep(400);
      await say('مزاعم مالية مع المرصد القابضة… وكلمة «ليث» تختفي من الشبكة كلما اقتربنا. ابحث أعمق في بطاقة مهند — حساباته القديمة وشبكته المنزلية.');
    },
    async router(){
      if(!S.flags.live)return;
      await sleep(400);
      await say('شبكته المنزلية MOHANNAD-HOME على راوتر RT-88 — الثغرة موثقة في 0day-souq منذ شهور. اخترق الراوتر… ثم جهازه… ثم خذ كل ما يجري عليه.');
    ...
    async finish(){
      ...
      await sleep(6000);
      await say('تم القبض على مهند الحسني. أمّن — في مكان آخر تماماً، بعيداً عن المستودع الذي أعطيتنا إياه.');
      await say('المستودع كان فارغاً يا راصد. إلا من صندوق واحد في المنتصف… عليه بطاقة موجهة إليك: «شكراً للراصد. كنت رائعاً. — ليث»');
