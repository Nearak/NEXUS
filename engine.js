'use strict';
/* ============================================================
   NEXUS-7 — engine.js v12.1 (كامل — نظام FACTS محصّن)
   1) قاعدة حقائق: كتابة مضمونة + فحص فوري ودوري 400ms
   2) جسر قصة موحد على ST_TEXT — بلا ST_BEATS
   3) استمرارية كاملة: F5/إغلاق في منتصف مهمة يستعيد كل شيء
   4) NET-3D: مشهد شبكي مجسّم واحد بنماذج عقد متمايزة
   ============================================================ */
...
const FACT_LABELS={helpUsed:'الإحاطة',nmapDone:'مسح الشبكة',hydraCracked:'اعتماد الصيانة',connected:'النفق المؤكد',logDownloaded:'سجل العبور',camDownloaded:'لقطة الكاميرا',frameAnalyzed:'تحليل اللقطة',plateIdentified:'مطابقة المالك',rootAchieved:'صلاحية الجذر',encDownloaded:'الملف المحمي',caseDecrypted:'فك الختم',boardLinks:'روابط الأدلة',reportSent:'رفع التقرير',mirqabScanned:'تشغيل الماسح',mqMohannad:'إشارة مهند',mqTriangulated:'تثليث الموقع',mqLayth:'إشارة ليث',mqDecoded:'فك الحزمة',lynxMohannad:'بصمة مهند',lynxLayth:'بصمة ليث',socialFound:'أرشفة الحساب',routerProfiled:'فحص الراوتر',cookiesStolen:'عزل الجلسة',cookiesDecrypted:'تحليل الجلسة',loginAuthenticated:'دخول الحساب',inboxRead:'تثبيت الرسالة',loggedIn:'الرسالة التشغيلية'};
function factSatisfied(goal,fact,want){
  const got=Facts.data[fact];
  return goal.min===true?(got||0)>=want:got===want;
}
...
const Facts={
  ...
  set(name,val){
    ...
    // أعد الفحص حتى عند تكرار القيمة: قد تكون الحقيقة صحيحة بينما كان الهدف
    // مقفلاً سابقاً أو آتياً من حفظ قديم، وهذا هو سبب خلل connect التاريخي.
    this.checkAll();
    return changed;
  },
  flush(){
  ...
  checkAll(options={}){
    ...
    const d=(typeof NIGHTS!=='undefined')?NIGHTS['n'+World.night]:null;
    if(!d||!d.goals)return;
    let changed=false;
    d.goals.forEach(g=>{
      const requirements=g.need||g.when;
      if(g.done||!requirements||!goalUnlocked(g,d))return;
      const ok=Object.entries(requirements).every(([fact,want])=>factSatisfied(g,fact,want));
      if(ok){
        g.done=true;changed=true;
        const li=$('#obj-'+g.id);if(li)li.classList.add('done');
        if(!silent)tone(740,.07,'triangle',.045);
        const dn=d.goals.filter(x=>x.done).length;
        if(!silent)toast('هدف مكتمل','سجل المهمة: '+dn+'/'+d.goals.length,'good');
        if(g.clues)g.clues.forEach(cid=>addClue(cid,silent));
        if(!silent)storyTrigger(g.id);
      }
    });
...
function goalUnlocked(goal,d=nightDef()){
  if(!goal||!d)return false;
  const deps=goal.after?([].concat(goal.after)):[];
  return deps.every(id=>{const g=d.goals.find(x=>x.id===id);return !!(g&&g.done);});
}
function isSendGoal(g){
  const requirements=g&&(g.need||g.when);
  return !!(g&&(g.id==='g_sent'||(requirements&&requirements.reportSent===true)));
}
function canSendNow(){const d=nightDef();return d&&d.goals?d.goals.every(g=>g.done||isSendGoal(g)):false;}
function missingForSend(){
  const d=nightDef();if(!d)return'—';
  const m=(d.goals||[]).filter(g=>!g.done&&!isSendGoal(g)).map(g=>(d.hints&&d.hints[g.id])?d.hints[g.id].split('—')[0].split('.')[0]:g.id);
  return m.length?m.join(' · '):'—';
}
function storyHint(){
  const d=nightDef();if(!d||!World.flags.live)return;
  const o=(d.goals||[]).find(g=>!g.done&&goalUnlocked(g,d));
  if(!o)return;
...
function renderObj(){
  ...
  $('#phObj').innerHTML=pool.length
    ?pool.map(o=>{
      const unlocked=goalUnlocked(o,d),current=!o.done&&unlocked&&!pool.some(x=>!x.done&&goalUnlocked(x,d)&&x!==o&&pool.indexOf(x)<pool.indexOf(o));
      const cls=o.done?'done':(!unlocked?'locked':(current?'current':''));
      const requirements=o.need||o.when||{};
      const checks=Object.entries(requirements).map(([fact,want])=>'<em class="req '+(factSatisfied(o,fact,want)?'ok':'')+'">'+(factSatisfied(o,fact,want)?'✓':'○')+' '+(FACT_LABELS[fact]||fact)+'</em>').join('');
      return '<li id="obj-'+o.id+'" class="'+cls+'"><span class="tick"></span><span><small>'+(o.phase||'مرحلة')+'</small>'+o.t+'<span class="reqs">'+checks+'</span></span></li>';
    }).join('')
    :'<li style="color:var(--muted)"><span class="tick"></span><span>لا مهمة نشطة — بانتظار اتصال المشرف…</span></li>';
  const counter=$('#missionCount');
  if(counter)counter.textContent=pool.filter(o=>o.done).length+'/'+pool.length;
  const dock=$('#missionDock');
  if(dock)dock.classList.toggle('complete',pool.length>0&&pool.every(o=>o.done));
}

let storyQueue=Promise.resolve();
function queueStory(lines,who){
  storyQueue=storyQueue.catch(()=>{}).then(()=>sayLines(lines,who));
  return storyQueue;
}

/* ============ جسر القصة — يشتعل عند شطب أهداف محددة ============ */
function storyTrigger(goalId){
  const key='n'+World.night;
  const T=ST_TEXT[key];if(!T)return;
  const map={
    n1:{g_brief:'recon',g_recon:'access',g_access:'identity',g_identity:'case'},
    n2:{g_signal:'grab1',g_fix:'fix',g_companion:'grab2',g_decode:'decoded'},
    n3:{g_lynx1:'dossiers',g_router:'router',g_crack:'cookies',g_login:'login'}
  };
  const m=map[key];if(!m)return;
  const textKey=m[goalId];
  if(textKey&&T[textKey]){
    queueStory(T[textKey]).catch(e=>console.error('[STORY:'+key+'/'+textKey+']',e));
  }
}
/* ============ العالم ============ */
const World={
  ...
  host:null,
  board:{},
  flags:{live:false,sessionStarted:0,nightDone:0,shipmentShown:0,n3ring:0,camShown:0,driverShown:0,frameAnalyzed:0,declined:0,routerCracked:0,hash:0,hydraDone:0,rootDone:false,caseDecrypted:false,cookiesDecrypted:false}
};
const S={trace:{on:false,pct:0,timer:null}};
...
const TERMS=[];
const CMD_LIST=['help','ls','cat','open','download','dl','rm','nmap','net','net3d','hydra','hashcat','john','connect','ssh','msfconsole','db','analyzer','decrypt','notes','evidence','board','mirqab','mrq','lynx','osint','disconnect','exit','trace','send','reboot','shutdown','clear','cls','whoami','date','pwd','echo','history','ping','ifconfig','netstat','ps','browser'];
...
function buildTerm(host){
  ...
  tprintTo(t,'NEXUS-7 secure shell — build 12.1','dim');
...
async function runCmd(raw,t){
  ...
  try{
    switch(cmd){
      ...
      case 'net':case 'net3d':openApp('net3d');break;
      case 'analyzer':openApp('analyzer');break;
      ...
      case 'reboot':await rebootSeq();break;
      case 'shutdown':shutdownSeq();break;
...
function cmdHelp(){
  const rows=[['── استطلاع ──',''],['nmap','مسح الشبكة'],['net3d','فتح الشبكة المجسمة'],['ping &lt;ip&gt;','اختبار وصول'],['── اختراق ──',''],['hydra -l admin -P rockyou.txt ssh://ip','كسر كلمة مرور SSH/HTTP'],['connect &lt;ip&gt; [user pass]','فتح نفق'],['msfconsole','إطار استغلال الثغرات'],['── ملفات ──',''],['ls · cat &lt;f&gt; · download &lt;f&gt; · rm &lt;f&gt;','استعراض ونسخ وحذف'],['── أدوات المشغّل ──',''],['db · analyzer · decrypt · evidence · mirqab · lynx · notes · send · trace','—'],['── النظام ──',''],['reboot · shutdown · ifconfig · netstat · ps · history · clear','—']];
...
async function cmdDownload(name){
  ...
  if(f.got){syncDownloadedFact(name);tprint(name+': already in vault — mission state verified','dim');return;}
  ...
  bumpTrace(10);
  syncDownloadedFact(name);
}
function syncDownloadedFact(name,value=true){
  if(name==='hwy16_log.log')Facts.set('logDownloaded',value);
  if(name==='cam04_frame.jpg'){Facts.set('camDownloaded',value);refreshAnalyzer();}
  if(name==='case_file.enc')Facts.set('encDownloaded',value);
}
...
async function cmdNmap(){
  if(World.scanDone){Facts.set('nmapDone',true);tprint('nmap: scan complete — mission state verified · افتح NET-3D لمراجعة العقد','dim');return;}
...
async function cmdHydra(arg){
  ...
  if(World.flags.hydraDone){Facts.set('hydraCracked',true);tprint('hydra: كُسرت سابقاً — mission state verified · admin : <span class="gr">orion2024</span>','dim');return;}
...
async function cmdConnect(ip,user,pass){
  ...
  if(World.host===ip){
    netSetActive(ip);
    if(World.flags.hydraDone)Facts.set('hydraCracked',true);
    Facts.set('connected',true);
    tprint('<span class="gr">connection verified</span> — tunnel to '+ip+' is active','dim');
    return;
  }
  ...
  if(!World.flags.hydraDone){
    if(user&&pass&&user.toLowerCase()==='admin'&&pass==='orion2024'){
      World.flags.hydraDone=1;persist();Facts.set('hydraCracked',true);tprint('ssh creds accepted for '+ip,'ok');
    }else{
  ...
  tprint('audit ticket :: OP-4471-A / scope=read-only-evidence','dim');
  tprint('opening maintenance tunnel :: ssh/22 → evidence relay/4471','dim');
...
async function runMsf(line,T){
  ...
  try{
    ...
    if(p==='exploit'||p==='run'){
      ...
      if(!World.flags.rootDone){World.flags.rootDone=true;persist();bumpTrace(15);toast('metasploit','جلسة root مفتوحة — الجذر لك.','good');}
...
async function cmdSend(){
  if(goalDone('g_sent')){
    if(World._finishing){tprint('supervisor debrief in progress — read the secure channel.','dim');return;}
    tprint('report already sent — preparing next operation.','dim');
    if(!World.flags.nightDone){World.flags.nightDone=1;persist();}
    offerNextNight();
    return;
  }
  if(!canSendNow()){
...
async function doSend(bundle){
  ...
  finishNight().catch(e=>console.error('[finish night]',e));
}
...
const FILES={
  'README.txt':{where:'local',kb:1,kind:'text',body:'NEXUS-7 :: INTEL WORKSTATION — BUILD 13.1\n----------------------------------------\nمحطة مشغّل في وحدة الاستخبارات.\n\nالأدوات:\n  TERM (نوافذ متعددة + Tab) · NET-3D · FILES · ANALYZER · EVIDENCE\n  MIRQAB (ليلة 2+) · LYNX (ليلة 3+) · DECRYPT · DB · BROWSER · NOTES'},
...
function renderTrash(){
  ...
  L.querySelectorAll('button').forEach(b=>{
    b.onclick=()=>{const n=World.trash.splice(+b.dataset.i,1)[0];World.files.push(n);FILES[n].got=true;syncDownloadedFact(n,true);renderTrash();renderFiles();persist();sfx.pop();};
  });
}
let fvToken=0;
...
function netReveal(ip){if(NET.known.has(ip))return;NET.known.add(ip);renderNet();}
function netSetActive(ip){NET.active=ip;renderNet();}
function renderNet(){
  const known=$('#netKnown');if(known)known.textContent=NET.known.size;
  const host=$('#nmHost');if(host)host.textContent=World.host?(World.host+' :: '+NET.data[World.host].label):'لا نفق نشط';
  const state=$('#netState');if(state)state.textContent=NET.active?'TUNNEL / ACTIVE':'PASSIVE / SCAN';
  const frame=$('#td3Frame');if(frame)buildNet3D(frame);
}
function netPulse(ip){
  const node=TD3.nodes.find(n=>n.userData.ip===ip);
  if(node)node.userData.pulseUntil=Date.now()+850;
}
function buildNet(host){
  host.innerHTML='<div class="t-net t-net3d">'+
    '<div class="nm-head"><span>NET-3D · AUTH SCOPE 7-A · 10.0.44.0/24</span><b id="nmHost">—</b></div>'+
    '<div class="net-status"><span>العقد المكتشفة <b id="netKnown">0</b>/4</span><span>الحالة <b id="netState">PASSIVE / SCAN</b></span><span>التذكرة <b>OP-4471-A</b></span></div>'+
    '<div class="net-pane net3d-pane"><div class="t3d-frame" id="td3Frame"></div><div class="model-key"><span>◆ المحطة</span><span>◎ البوابة</span><span>▣ الجسر</span><span>◉ الكاميرا</span></div></div>'+
    '<div class="nm-foot">اسحب للدوران · عجلة الفأرة للتقريب · انقر نموذجًا للاتصال · شغّل <code>nmap</code> لاكتشاف العقد</div></div>';
  renderNet();
}
...
async function breach(){
  ...
  if(T&&T.breach&&World.flags.live)queueStory(T.breach).catch(()=>{});
}
function hostDown(){
...
/* ============ ANALYZER — مختبر الأدلة البصرية ============ */
let analyzerBusy=false;
function buildAnalyzer(host){
  host.innerHTML='<div class="t-analyzer">'+
    '<div class="an-head"><div><b>VISUAL EVIDENCE LAB</b><span>OP-4471-A / CAM-04 / CHAIN OF CUSTODY</span></div><i id="anBadge">AWAITING SOURCE</i></div>'+
    '<div id="anEmpty" class="fv-empty">نزّل <code>cam04_frame.jpg</code> من عقدة HWY16-CAM أولًا.</div>'+
    '<div id="anWork" class="an-work" hidden><section class="an-stage"><div class="an-reticle"></div><img id="anImage" src="'+IMG.camCar+'" alt="لقطة كاميرا الطريق 16"><div class="an-stamp">CAM-04 · 2024-05-15 · 14:32:17Z</div></section>'+
    '<aside class="an-controls"><label>تكبير بصري <output id="anZoomVal">100%</output><input id="anZoom" type="range" min="100" max="180" value="100"></label><label>تباين <output id="anContrastVal">100%</output><input id="anContrast" type="range" min="80" max="180" value="100"></label>'+
    '<div class="an-meta"><span>المصدر<b>HWY16 / CHK-3</b></span><span>SHA-256<b>9B71…44A2</b></span><span>الحفظ<b>READ-ONLY</b></span></div><button id="anRun">تشغيل استخراج ANPR</button></aside>'+
    '<div id="anResult" class="an-result" hidden><span>ANPR MATCH</span><b>HX-4471</b><i>ثقة 96.8% · سيدان زرقاء · مسار 2</i></div></div></div>';
  host.querySelector('#anZoom').addEventListener('input',updateAnalyzerImage);
  host.querySelector('#anContrast').addEventListener('input',updateAnalyzerImage);
  host.querySelector('#anRun').addEventListener('click',analyzeFrame);
  refreshAnalyzer();
}
function updateAnalyzerImage(){
  const img=$('#anImage'),zoom=$('#anZoom'),contrast=$('#anContrast');if(!img||!zoom||!contrast)return;
  img.style.transform='scale('+(+zoom.value/100)+')';img.style.filter='contrast('+(+contrast.value/100)+') saturate(.82)';
  $('#anZoomVal').textContent=zoom.value+'%';$('#anContrastVal').textContent=contrast.value+'%';
}
function refreshAnalyzer(){
  const empty=$('#anEmpty'),work=$('#anWork');if(!empty||!work)return;
  const ready=!!FILES['cam04_frame.jpg'].got;
  empty.hidden=ready;work.hidden=!ready;
  const done=!!World.flags.frameAnalyzed;
  const badge=$('#anBadge'),result=$('#anResult'),btn=$('#anRun');
  if(badge){badge.textContent=done?'EVIDENCE VERIFIED':(ready?'SOURCE READY':'AWAITING SOURCE');badge.classList.toggle('ok',done);}
  if(result)result.hidden=!done;
  if(btn){btn.disabled=done;btn.textContent=done?'تم التحقق ✓':'تشغيل استخراج ANPR';}
  if(done)Facts.set('frameAnalyzed',true);
  updateAnalyzerImage();
}
async function analyzeFrame(){
  if(analyzerBusy)return;
  if(!FILES['cam04_frame.jpg'].got){toast('ANALYZER','اللقطة غير موجودة في الخزنة.','bad');sfx.err();return;}
  if(World.flags.frameAnalyzed){Facts.set('frameAnalyzed',true);refreshAnalyzer();return;}
  analyzerBusy=true;const btn=$('#anRun');if(btn){btn.disabled=true;btn.textContent='جارٍ عزل اللوحة…';}
  for(const msg of ['تثبيت الإطار','تصحيح المنظور','قراءة المحارف','مطابقة التوقيت']){
    const badge=$('#anBadge');if(badge)badge.textContent=msg+'…';
    sfx.tick();await sleep(360);
  }
  World.flags.frameAnalyzed=1;persist();Facts.set('frameAnalyzed',true);
  analyzerBusy=false;refreshAnalyzer();sfx.ok();toast('ANALYZER','استُخرجت اللوحة HX-4471 بثقة 96.8%.','good');
}
...
function initDecrypt(){
  ...
  const caseReady=src.which==='case'&&!Facts.get('caseDecrypted')&&FILES['case_file.enc'].got;
...
async function doSearch(){
  ...
  try{
    ...
    if(res.some(r=>r.plate==='HX-4471')&&!Facts.get('plateIdentified'))
...
function mqTargetForNight(){
  if(World.night===2)return Facts.get('mqMohannad')?'layth':'mohannad';
...
function buildMirqab(host){
  ...
  host.innerHTML='<div class="t-mrq">'+
    '<div class="mq-head"><b>مِرقاب — اعتراض وتحليل الإشارات</b><span class="mq-case">CASE 4471-B</span><span class="st" id="mqSt">خامل</span></div>'+
    ...
    '<div id="mqWork">'+
      ...
      '<div class="mq-intel" id="mqIntel"></div>'+
  ...
  $('#mqPower').onclick=()=>{
    ...
    if(!MQ.tune)MQ.tune=92;
    $('#mqTune').value=MQ.tune;
    onTune();
  ...
  $('#mqGrab').onclick=()=>{
    const entry=Object.entries(MQ_TARGETS).find(([k,v])=>v.freq===MQ.target);
    if(!entry)return;
    if(MQ.grabbed[entry[0]]){
      if(entry[0]==='mohannad')Facts.set('mqMohannad',true);
      if(entry[0]==='layth')Facts.set('mqLayth',true);
      toast('مِرقاب','الإشارة محفوظة — تمت مزامنة المهمة.','good');
      return;
    }
  ...
  if(Facts.get('mirqabScanned'))$('#mqPower').click();
  else refreshMqIntel();
}
function mqLog(html){
...
function refreshMqIntel(){
  const p=$('#mqIntel');if(!p)return;
  const moh=!!(MQ.grabbed.mohannad||Facts.get('mqMohannad'));
  const lay=!!(MQ.grabbed.layth||Facts.get('mqLayth'));
  const tri=!!Facts.get('mqTriangulated'),dec=!!Facts.get('mqDecoded');
  const count=[moh,tri,lay,dec].filter(Boolean).length;
  let h='<div class="mqi-head"><b>INTEL PIPELINE</b><span>'+count+'/4</span></div>';
  if(!moh)h+='<p>التقط حامل الإشارة الأول لفتح أدوات تحديد المصدر.</p>';
  else{
    h+='<div class="mqi-card '+(tri?'done':'')+'"><b>01 · تثليث المصدر</b><span>'+(tri?'DEAD-ZONE-K9 · دقة 18m':'قارن قوة الإشارة بين القطاعات الثلاثة')+'</span>';
    if(!tri)h+='<div class="mqi-zones"><button data-zone="N4">قطاع N4</button><button data-zone="K9">قطاع K9</button><button data-zone="E2">قطاع E2</button></div>';
    h+='</div>';
  }
  if(moh)h+='<div class="mqi-card '+(lay?'done':'')+'"><b>02 · القناة المرافقة</b><span>'+(lay?'UNKNOWN-DEV / contact=LAYTH':'اضبط 104.2 MHz والتقط الحامل الثانوي')+'</span></div>';
  if(moh&&lay)h+='<div class="mqi-card '+(dec?'done':'')+'"><b>03 · الحزمة الصوتية</b><span>'+(dec?'«أنا شاهد… ملفات المرصد تثبت كل شيء»':'الحزمة متداخلة؛ افصل القناتين لاستخراج الصوت')+'</span>'+(dec?'':'<button id="mqDecode">فصل القناتين وفك الحزمة</button>')+'</div>';
  p.innerHTML=h;
  p.querySelectorAll('[data-zone]').forEach(b=>b.onclick=()=>{
    if(b.dataset.zone!=='K9'){b.classList.add('bad');sfx.err();mqLog('<span class="rd">[BEARING]</span> '+b.dataset.zone+' :: multipath error / rejected');return;}
    Facts.set('mqTriangulated',true);sfx.ok();mqLog('<span class="sig">[FIX]</span> DEAD-ZONE-K9 :: triangulation radius 18m');addClue('fix');refreshMqIntel();
  });
  const d=$('#mqDecode');if(d)d.onclick=async()=>{
    d.disabled=true;d.textContent='جارٍ فصل القناتين…';
    await sleep(900);Facts.set('mqDecoded',true);addClue('witness');sfx.unlock();
    mqLog('<span class="sig">[VOICE]</span> "I am a witness. Al-Marsad ledgers prove everything."');refreshMqIntel();
  };
}
...
async function doLynx(q){
  ...
  res.querySelectorAll('.lx-item').forEach(el=>{
    el.onclick=()=>{
      ...
      if(!h.deep)return;
      if(h.deep==='social'){
        openApp('browser');brGo('social.mohannad');
      }
      if(h.deep==='router'){openApp('browser');brGo('mohannad-home.net');}
      if(h.deep==='thorn'){addClue('thorn');openApp('browser');brGo('thorn.trace');}
    };
  });
...
const PAGES={
  ...
  'social.mohannad':()=>'<div class="web-kicker">SOCIAL ARCHIVE / READ-ONLY</div><h1>@m_hussein77 — الحساب الاجتماعي</h1>'+
    '<div class="web-evidence"><b>تناقض زمني</b><span>الحساب مجمّد منذ «الوفاة» المعلنة، لكن سُجلت متابعة جديدة بعد الوفاة بثلاثة أيام.</span><i>confidence 87% · archive snapshot 6F2A</i></div>'+
    '<p>وجود النشاط لا يثبت هوية المستخدم وحده؛ احفظ الصفحة كمصدر ثم ابحث عن الجهاز الذي يحمل الجلسة.</p>'+
    (Facts.get('socialFound')?'<span class="web-verified">✓ أُرشفت الصفحة ضمن القضية</span>':'<span class="wlink primary" data-u="__archive_social">أرشفة الصفحة كسجل قضية</span>')+
  ...
  'social.mohannad/inbox':()=>'<div class="web-kicker">AUTHENTICATED SESSION / EVIDENCE MODE</div><h1>الرسائل — @m_hussein77</h1>'+
    ...
    (Facts.get('inboxRead')?'<span class="web-verified">✓ الرسالة مثبتة مع الطابع الزمني</span>':'<span class="wlink primary" data-u="__archive_inbox">تثبيت الرسالة كدليل تشغيلي</span>'),
  'mohannad-home.net':()=>'<div class="web-kicker">LOCAL DEVICE / 192.168.88.1</div><h1>RT-88 :: لوحة إدارة الراوتر</h1>'+
    (!Facts.get('routerProfiled')
      ?'<p>الخدمة تستجيب، لكن لا توجد بصمة موثقة بعد. افحص المنافذ والإصدار قبل اختيار أداة الدخول.</p><span class="wlink primary" data-u="__probe_router">فحص الخدمات وتثبيت البصمة</span>'
      :'<div class="web-evidence"><b>RT-88 firmware 2.4</b><span>80/tcp HTTP Admin · 1900/tcp UPnP · CVE-2024-8812</span><i>risk high · credential gate detected</i></div><p>شبكة MOHANNAD-HOME. الدخول الإداري يحتاج اعتمادًا قبل عزل الجلسة.</p>'+
        (World.flags.routerCracked
          ?'<span class="wlink primary" data-u="__hijack">عزل جلسة الجهاز وحفظ الحزمة</span>'
          :'<span class="wlink" data-u="__hydra_router">تشغيل hydra على إدارة الراوتر</span><p style="color:var(--muted);font-size:12px">بعد نجاح الأداة ارجع لهذه الصفحة.</p>')),
...
function attachLoginIfPresent(){
  ...
  b.onclick=()=>{
    ...
    if(u.value.trim()==='m_hussein77'&&p.value==='@m_hussein77'){
      ...
      sfx.unlock();glitch(1);
      Facts.set('loginAuthenticated',true);
...
function pageAction(act){
  ...
  if(act==='__archive_social'){
    Facts.set('socialFound',true);sfx.ok();toast('المتصفح','حُفظت الصفحة مع بصمتها الزمنية.','good');brGo('social.mohannad',false);return;
  }
  if(act==='__probe_router'){
    Facts.set('routerProfiled',true);addClue('router');sfx.ok();toast('المتصفح','ثُبتت بصمة RT-88 والخدمات المكشوفة.','good');brGo('mohannad-home.net',false);return;
  }
  if(act==='__archive_inbox'){
    Facts.set('inboxRead',true);Facts.set('loggedIn',true);sfx.ok();toast('المتصفح','ثُبتت رسالة المستودع ب ضمن القضية.','good');brGo('social.mohannad/inbox',false);return;
  }
...
function brGo(url,push=true){
  ...
  $('#brUrl').value=url;
  const sec=$('#brSecurity');if(sec){const local=url.startsWith('nexus://')||url.endsWith('.net');sec.textContent=local?'ISOLATED':'ARCHIVE';sec.classList.toggle('warn',url.includes('onion'));}
  const crumb=$('#brCrumb');if(crumb)crumb.textContent=url;
...
function buildBrowser(host){
  host.innerHTML='<div class="t-browser"><div class="br-bar"><button id="brBack" title="رجوع">‹</button><button id="brHome" title="الرئيسية">⌂</button><input id="brUrl" placeholder="nexus://home"><span id="brSecurity">ISOLATED</span><button id="brGoBtn" title="اذهب">→</button></div>'+
    '<div class="br-bookmarks"><span id="brCrumb">nexus://home</span><div><button data-u="social.mohannad">SOCIAL</button><button data-u="mohannad-home.net">RT-88</button><button data-u="meridian-holdings.com">MERIDIAN</button></div></div><div id="brBody"></div></div>';
  host.querySelector('#brBack').onclick=()=>{if(brHist.length>1){brHist.pop();brGo(brHist[brHist.length-1],false);}};
  host.querySelector('#brHome').onclick=()=>brGo('nexus://home');
  ...
  host.querySelectorAll('.br-bookmarks button').forEach(b=>b.onclick=()=>brGo(b.dataset.u));
...
function buildInfo(host){
  ...
  '<p><b>المنهج:</b> نفّذ المرحلة الحالية فقط، تحقق من نتيجة الأداة، ثم راجع زر «المهمة». المراحل المقفلة تُفتح تلقائيًا عند اكتمال ما قبلها.</p>'+
  '<p><b>NET-3D:</b> المشهد الشبكي الوحيد؛ يعرض المحطة والبوابة والجسر والكاميرا كنماذج مجسّمة قابلة للفحص والاتصال.</p>'+
  '<p><b>ANALYZER:</b> مختبر أدلة يحفظ سلسلة الحيازة ويستخرج اللوحات من اللقطات قبل مطابقتها مع السجل المدني.</p>'+
  '<p><b>استعادة الجلسة:</b> إذا حدّثت الصفحة، يعيد النظام بناء العقد والملفات والنفق وحقائق المهمة من آخر حفظ.</p></div>';
}
...
const IC={
  ...
  analyzer:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 7V3h4M17 3h4v4M21 17v4h-4M7 21H3v-4"/><circle cx="12" cy="12" r="4"/><path d="M12 8v8M8 12h8"/></svg>',
...
const APPS={
  ...
  browser:{title:'NEXUS WEB — متصفح التحقيق',w:800,h:570,icon:IC.browser,build:buildBrowser},
  ...
  net3d:{title:'NET-3D — الشبكة المجسمة',w:780,h:580,icon:IC.net,build:buildNet,onOpen:renderNet},
  analyzer:{title:'ANALYZER — تحليل الأدلة',w:780,h:570,icon:IC.analyzer,build:buildAnalyzer,onOpen:refreshAnalyzer},
  ...
  mirqab:{title:'MIRQAB — مِرقاب',w:760,h:620,icon:IC.mirqab,build:buildMirqab,onOpen:refreshMqIntel},
...
function openMission(){
  phoneUnfold();
  $('#tabMission').classList.add('on');$('#tabChat').classList.remove('on');
  renderObj();setPhoneView('mission');
}
function resetPhoneIdle(){
 ...
 $('#tabMission').onclick=openMission;
 $('#missionDock').onclick=openMission;
...
async function say(text,who){
  ...
  // امنح اللاعب وقتاً بعد ظهور الرسالة، لا أثناء مؤشر الكتابة فقط.
  await sleep(900+Math.min(2600,text.length*22));
}
...
function offerNextNight(){
  const q=$('#phChips');if(!q)return;
  phoneUnfold();setPhoneView('chat');
  $('#tabChat').classList.add('on');$('#tabMission').classList.remove('on');q.innerHTML='';
  const b=document.createElement('button');
  b.className='pchip send next-night';b.textContent='ابدأ الليلة التالية';
  b.onclick=()=>{b.disabled=true;b.textContent='جارٍ تجهيز العملية…';nightTransitionFlow();};
  q.appendChild(b);
  toast('المهمة مكتملة','اقرأ الرسائل، ثم ابدأ الليلة التالية عندما تكون جاهزًا.','good');
}
let ringIv=null;
function incomingCall(){
  /* ليلة 3: رنّة واحدة تنقطع ثم رسالة نصية */
  if(World.night===3&&!World.flags.n3ring){
    ...
    setTimeout(()=>{
      ...
      setTimeout(async()=>{
        ...
        await queueStory((ST_TEXT.n3&&ST_TEXT.n3.ringMsg)||[]);
        setPhoneView('chat');
      },1500);
    },6000);
    return;
  }
  /* استمرارية: مهمة جارية مسبقاً → استئناف القناة دون رنّة ترحيب */
  if(World.flags.sessionStarted&&!World.flags.nightDone&&(typeof NIGHTS!=='undefined'||NIGHTS['n'+World.night])){
    World.flags.live=true;
    renderObj();refreshQuick();
    sysSay('استُؤديت الجلسة — القناة مفتوحة، المهمة قائمة');
    setPhoneView('chat');
    return;
  }
  if(World.flags.nightDone||!(typeof NIGHTS!=='undefined'&&NIGHTS['n'+World.night])){
    if(World.flags.nightDone){
      if(World.flags.nightDone===2)setTimeout(nightTransitionFlow,500);
      else offerNextNight();
    }
    return;
  }
...
function stopRing(){
 ...
 $('#phAccept').onclick=()=>{
  ...
  if(T&&T.start){
    ...
    queueStory(T.start).catch(e=>console.error('[STORY:start]',e));
  }
};
 $('#phDecline').onclick=()=>{
  ...
  if(World.flags.declined===1)setTimeout(()=>{if(World.flags.live)beat('decline');},1800);
...
function beat(name){
  ...
  if(name==='start'){
    World.flags.live=true;
    renderObj();refreshQuick();setPhoneView('chat');
    return queueStory(T.start).catch(e=>console.error('[STORY:start]',e));
  }
  if(T[name]&&World.flags.live)return queueStory(T[name]).catch(e=>console.error('[STORY:'+key+'/'+name+']',e));
}

/* ============ نهاية الليلة → 24 ساعة → التالية ============ */
async function finishNight(){
  if(World._finishing)return;
  World._finishing=true;
  // احفظ اكتمال الليلة قبل حوار النهاية كي يستطيع التحديث استئناف الانتقال.
  World.flags.nightDone=1;persist();
  if(World.flags.live){
    const T=ST_TEXT['n'+World.night];
    if(T&&T.finish)await queueStory(T.finish);
    sysSay('المشرف أغلق القناة');
  }
  if(World.host){hostDown();tprint('connection closed by remote host','dim');}
  World.flags.live=false;
  World._finishing=false;
  persist();
  offerNextNight();
}
...
async function nightTransitionFlow(){
  ...
  // nightDone=2 يعني أن الصفحة أُعيد تحميلها أثناء مشهد الانتقال؛ أكمل بدل التعليق.
  if(!World.flags.nightDone){World._transitioning=false;return;}
  if(World.flags.nightDone===1){World.flags.nightDone=2;persist();}
...
function persist(){
  try{
    ...
    localStorage.setItem('nexus7',JSON.stringify({
      ...
      board:World.board,
      facts:Facts.data,
      mq:{tune:MQ.tune,grabbed:MQ.grabbed},
      flags:{live:0,sessionStarted:World.flags.sessionStarted,
             ...
             camShown:World.flags.camShown,driverShown:World.flags.driverShown,frameAnalyzed:World.flags.frameAnalyzed,
...
function reconcileProgress(){
  const set=(name,value)=>{if(value)Facts.data[name]=true;};
  if(World.scanDone){
    NET.known.add('10.0.44.23');NET.known.add('10.0.44.77');
    set('nmapDone',true);
  }
  set('hydraCracked',World.flags.hydraDone);
  set('connected',!!World.host);
  set('logDownloaded',FILES['hwy16_log.log'].got);
  set('camDownloaded',FILES['cam04_frame.jpg'].got);
  set('encDownloaded',FILES['case_file.enc'].got);
  set('frameAnalyzed',World.flags.frameAnalyzed);
  set('plateIdentified',World.flags.driverShown);
  set('rootAchieved',World.flags.rootDone);
  set('caseDecrypted',World.flags.caseDecrypted);
  set('cookiesDecrypted',World.flags.cookiesDecrypted);
  // توافق مع حفظ أقدم: عزل الجلسة يعني أن فحص الراوتر سبقها، وloggedIn كان
  // يعني سابقاً فتح الرسالة أيضاً.
  if(Facts.data.cookiesStolen)set('routerProfiled',true);
  if(Facts.data.loggedIn){set('loginAuthenticated',true);set('inboxRead',true);}
  set('mqMohannad',MQ.grabbed.mohannad);
  set('mqLayth',MQ.grabbed.layth);
  const board=World.board[World.night];
  if(board&&Array.isArray(board.links))Facts.data.boardLinks=board.links.length;
}
function restoreGame(){
  ...
  if(sv.host&&NET.data[sv.host]){World.host=sv.host;NET.known.add(sv.host);NET.active=sv.host;}
  if(sv.board&&typeof sv.board==='object')World.board=sv.board;
  if(sv.facts&&typeof sv.facts==='object')Facts.data=sv.facts;
  if(sv.mq&&typeof sv.mq==='object'){
    if(Number.isFinite(+sv.mq.tune))MQ.tune=+sv.mq.tune;
    if(sv.mq.grabbed&&typeof sv.mq.grabbed==='object')MQ.grabbed=sv.mq.grabbed;
  }
  ...
  reconcileProgress();
  Facts.checkAll({silent:true});
  // حفظ قديم قد يحتوي هدف الإرسال مكتملًا دون علم نهاية الليلة.
  if(goalDone('g_sent')&&!World.flags.nightDone){World.flags.nightDone=1;persist();}
}
...
function setupStart(){
  ...
  scr.hidden=false;
  setTimeout(()=>$('#suName').focus(),0);
  const btn=$('#suGo');
  const go=()=>{
    const n=$('#suName').value.trim();
    const p=$('#suPass').value, p2=$('#suPass2').value;
    const err=$('#suErr');
    if(!/^[\p{L}\p{N}_-]{3,16}$/u.test(n)){err.textContent='الاسم: من 3 إلى 16 حرفاً أو رقماً، بلا مسافات.';sfx.err();return;}
  ...
  btn.onclick=go;
  ['#suName','#suPass','#suPass2'].forEach(sel=>$(sel).addEventListener('input',()=>{$('#suErr').textContent='';}));
...
function loginStart(saved){
  ...
  btn.onclick=go;
  $('#lgPass').addEventListener('input',()=>{$('#lgErr').textContent='';});
...
function resetAll(advanceNight){
  ...
  World.flags={live:false,sessionStarted:0,nightDone:0,shipmentShown:0,n3ring:0,camShown:0,driverShown:0,frameAnalyzed:0,declined:0,routerCracked:0,hash:0,hydraDone:0,rootDone:false,caseDecrypted:false,cookiesDecrypted:false};
  ...
  Facts.reset();
  World._finishing=false;World._transitioning=false;
  NET.known=new Set(['SELF','10.0.44.1']);NET.active=null;
  closeAllWins();
...
async function rebootSeq(){
  if(World._rebooting)return;
  World._rebooting=true;
  // أي قيمة موجبة تعني أن الليلة انتهت، بما فيها الحالة 2 المحفوظة أثناء الانتقال.
  const advance=!!World.flags.nightDone;
  sfx.shutdown();
  toast('النظام','إعادة التشغيل…');
  const d=$('#desktop');
  d.classList.add('off');
  try{
    await sleep(780);
    ...
    if(advance&&has&&World.night===2&&!World.flags.shipmentShown){
      ...
      setTimeout(incomingCall,advance&&has?1800:2500);
    }
  }catch(e){
    console.error('[reboot]',e);d.classList.remove('off');toast('إعادة التشغيل','تعذّر إكمال الإقلاع — حاول مرة أخرى.','bad');
  }finally{World._rebooting=false;}
}
 $('#powerOnBtn').onclick=()=>{
  audioInit();
  $('#powerScreen').hidden=true;
  const d=$('#desktop');d.hidden=false;
  if(World.flags.nightDone){resetAll(true);}
  else{resetVisualOnly();persist();}
  d.classList.remove('boot-in');void d.offsetWidth;
  d.classList.add('boot-in');
  sfx.conn();
 ...
 $('#pdReboot').onclick=()=>{$('#powerDlg').hidden=true;rebootSeq().catch(e=>console.error('[reboot button]',e));};
 $('#smShutdown').onclick=()=>{$('#startMenu').hidden=true;$('#powerDlg').hidden=false;};
 $('#smReboot').onclick=()=>{$('#startMenu').hidden=true;rebootSeq().catch(e=>console.error('[reboot menu]',e));};
...
const ICON_BASE=['terminal','files','browser','notes','music','lynx','net3d','analyzer','db','board','mirqab','decrypt','trash'];
...
document.addEventListener('keydown',e=>{
  const typing=e.target&&e.target.matches('input,textarea,[contenteditable="true"]');
  if(e.altKey&&e.key.toLowerCase()==='m'){
    e.preventDefault();openMission();return;
  }
  if(e.key!=='Escape'||typing)return;
  if(!$('#photoModal').hidden){$('#photoModal').hidden=true;return;}
  if(!$('#powerDlg').hidden){$('#powerDlg').hidden=true;return;}
  if(!$('#startMenu').hidden){$('#startMenu').hidden=true;return;}
  if(!$('#ctxMenu').hidden){$('#ctxMenu').hidden=true;return;}
  if(focusId&&wins[focusId]&&wins[focusId].style.display!=='none'){
    wins[focusId].style.display='none';syncTaskWindows();
  }
});
...
/* ============ المشهد المجسّم NET-3D ============ */
const TD3={scene:null,cam:null,ren:null,nodes:[],links:[],parts:[],stars:[],ray:null,mouse:null,signature:'',generation:0,rotY:0,rotX:0.25,zoom:130,isDown:false,px:0,py:0};
function buildNet3D(host){
  if(!host)return;
  if(typeof THREE==='undefined'){
    host.innerHTML='<div class="fv-empty">تعذّر تحميل محرّك NET-3D. يمكنك متابعة المهمة من الطرفية ثم إعادة فتح الأداة.</div>';
    return;
  }
  TD3.mouse=new THREE.Vector2();
  initNet3D(host);
}
function initNet3D(frame){
  const signature=[...NET.known].sort().join('|')+'::'+(NET.active||'');
  if(TD3.ren&&TD3.ren.domElement.parentElement===frame&&TD3.signature===signature)return;
  TD3.signature=signature;const generation=++TD3.generation;
  if(TD3.ren){try{TD3.ren.dispose();TD3.ren.domElement.remove();}catch(e){}}
  frame.innerHTML='';TD3.parts=[];
  ...
  Object.keys(NET.data).forEach((ip,i)=>{
    ...
    let core;
    if(d.self){
      core=new THREE.Mesh(new THREE.OctahedronGeometry(7,1),mat);
      const ring=new THREE.Mesh(new THREE.TorusGeometry(10,0.45,8,40),new THREE.MeshBasicMaterial({color:col,transparent:true,opacity:.65}));
      ring.rotation.x=Math.PI/2;mesh.add(core,ring);
    }else if(ip==='10.0.44.77'){
      core=new THREE.Mesh(new THREE.BoxGeometry(12,7,8),mat);mesh.add(core);
      const lens=new THREE.Mesh(new THREE.CylinderGeometry(3.3,4.2,5,24),mat);lens.rotation.x=Math.PI/2;lens.position.z=6;mesh.add(lens);
      const mount=new THREE.Mesh(new THREE.CylinderGeometry(1.1,1.6,8,12),mat);mount.position.y=-7;mesh.add(mount);
    }else if(ip==='10.0.44.1'){
      core=new THREE.Mesh(new THREE.TorusKnotGeometry(4.8,1.5,70,10),mat);mesh.add(core);
    }else{
      core=new THREE.Mesh(new THREE.BoxGeometry(10,10,10,2,2,2),mat);mesh.add(core);
      const ring=new THREE.Mesh(new THREE.TorusGeometry(8,0.5,8,32),new THREE.MeshBasicMaterial({color:col,transparent:true,opacity:.5}));
      ring.rotation.y=Math.PI/2;mesh.add(ring);
    }
    mesh.position.set(p[0],p[1],p[2]);
    mesh.userData={ip:ip,label:d.label,known:known,locked:d.locked,self:d.self};
    mesh.traverse(o=>{o.userData.ip=ip;});
    if(known){
  ...
  dom.addEventListener('click',()=>{
    ...
    const hits=TD3.ray.intersectObjects(TD3.nodes,true);
    if(hits.length){
      const ip=hits[0].object.userData.ip;
      const n=TD3.nodes.find(x=>x.userData.ip===ip);
      if(!n)return;
  ...
  (function loop3d(){
    if(generation!==TD3.generation)return;
    ...
    TD3.nodes.forEach(n=>{
      ...
      const pulsing=(n.userData.pulseUntil||0)>Date.now();
      const s=n.userData.self?1:(pulsing?1.24+Math.sin(Date.now()/70)*0.12:(act?1.14+Math.sin(Date.now()/170)*0.13:1));
      n.scale.setScalar(s);
...
function initIntro(){
  ...
  vid.addEventListener('error',toBoot,{once:true});
  let introSeen=false;
  try{introSeen=sessionStorage.getItem('nexus7_intro')==='1';}catch(e){}
  if(introSeen){ try{scr.remove();}catch(e){} afterIntro(); return; }
  scr.hidden=false;
  playBtn.onclick=()=>{
    audioInit();
    vid.src=INTRO_URL;
...
function enterOS(){
  ...
  requestAnimationFrame(drawMq);
  Facts.ready=true;Facts.flush();Facts.checkAll();
  sysSay('القناة خاملة — بانتظار اتصال المشرف');
  setTimeout(()=>toast('نظام','أهلاً '+ID.name+' — الوحدة بانتظارك. رمزك: راصد.'),700);
  // استئناف انتقال كان قد انقطع بسبب تحديث الصفحة، وإلا انتظر الاتصال المعتاد.
  if(World.flags.nightDone===2)setTimeout(nightTransitionFlow,900);
  else if(World.flags.nightDone===1)setTimeout(offerNextNight,900);
  else setTimeout(incomingCall,3500);
}
